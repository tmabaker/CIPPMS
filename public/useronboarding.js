// =============================================
// MULTI-TENANT — NOIT Client User Onboarding
// =============================================
var TENANTS = [
  ...
  { name: 'Community Association Management', domain: 'camgmttlc.com', id: '3cbaf35e-e56c-4aee-993a-ddb12651da43' },
...
var currentTenant = null; // { name, domain, id }

// =============================================
// DATA STORE (per-tenant)
// =============================================
var GLOBAL_KEY = 'noit_userform_global';

var defaultData = {
  companies:   [],
  domains:     [],
  departments: ['Operations','Administration','Sales','Finance','IT','HR','Marketing'],
  jobTitles:   ['Administrator','Analyst','Coordinator','Director','Manager','Supervisor'],
...
function tenantStoreKey(){
  return currentTenant ? 'noit_userform_' + currentTenant.id : null;
}

function loadTenantData(){
  var key = tenantStoreKey();
  if (!key) { data = JSON.parse(JSON.stringify(defaultData)); return; }
  try {
    var _s = localStorage.getItem(key);
  ...
  // Ensure tenant domain is present
  if (currentTenant && data.domains.indexOf(currentTenant.domain) === -1) {
    data.domains.unshift(currentTenant.domain);
    data.domains.sort();
    save();
  }
  // If no company, add tenant name
  if (currentTenant && data.companies.length === 0) {
    data.companies.push(currentTenant.name);
    save();
  }
}
...
function onTenantChange(){
  ...
  if (!tid) {
    ...
    document.getElementById('tenantCardName').textContent = 'No tenant selected';
    document.getElementById('tenantCardDomain').textContent = '—';
    document.getElementById('tenantCardStatus').innerHTML = '<span style="font-size:11px;background:rgba(74,80,112,.12);border:1px solid rgba(74,80,112,.3);color:var(--text-dim);padding:3px 10px;border-radius:20px;">&#9679; Select tenant above</span>';
    document.getElementById('headerTenant').textContent = '';
    setText('prev-tenant', '—');
    document.getElementById('adminTenantInfo').textContent = 'Select a tenant above to manage its lists.';
    document.getElementById('adminContent').classList.add('disabled-overlay');
    return;
  }

  currentTenant = TENANTS.find(function(t){ return t.id === tid; });
  if (!currentTenant) return;

  loadTenantData();
  setFormEnabled(true);

  document.getElementById('tenantCardName').textContent = currentTenant.name;
  document.getElementById('tenantCardDomain').textContent = currentTenant.domain;
  document.getElementById('tenantCardStatus').innerHTML = '<span style="font-size:11px;background:rgba(62,207,142,.12);border:1px solid rgba(62,207,142,.3);color:var(--success);padding:3px 10px;border-radius:20px;">&#9679; Active</span>';
  document.getElementById('headerTenant').textContent = '— ' + currentTenant.name;
  setText('prev-tenant', currentTenant.name);

  document.getElementById('adminTenantInfo').innerHTML = 'Managing lists for <strong>' + escHtml(currentTenant.name) + '</strong> (' + escHtml(currentTenant.domain) + ')';
  document.getElementById('adminContent').classList.remove('disabled-overlay');
...
function setFormEnabled(enabled){
  ...
  if (enabled) {
    fields.classList.remove('disabled-overlay');
    btn.disabled = false;
  } else {
    fields.classList.add('disabled-overlay');
    btn.disabled = true;
  }
...
function submitToCIPP(){
  if (!currentTenant) { showResult('error', 'No Tenant Selected', 'Please select a tenant from the dropdown above.'); return; }

  var TENANT_ID     = currentTenant.id;
  var TENANT_DOMAIN = currentTenant.domain;
  var TENANT_NAME   = currentTenant.name;
...
function saveAndTest(){
  ...
  cippFetch('/api/ListTenants', 'GET')
    ...
    .then(function(){
      var msg = 'Connected!';
      if (currentTenant) msg += ' Loading licenses for ' + currentTenant.name + '...';
      showConnResult('success', msg);
      setStatus('connected', 'Connected');
      if (currentTenant) loadLicenses(currentTenant.id);
    })
...
function buildUPN(first, last){
  ...
  var domain = domEl && domEl.value ? domEl.value : (currentTenant ? currentTenant.domain : 'domain.com');
...
function exportLists(){
  if (!currentTenant){ showToast('Select a tenant first', true); return; }
  var json = JSON.stringify(data, null, 2);
  var blob = new Blob([json], { type: 'application/json' });
  var url  = URL.createObjectURL(blob);
  var a    = document.createElement('a');
  a.href     = url;
  a.download = 'NOIT-' + currentTenant.name.replace(/\s+/g, '-') + '-lists-' + new Date().toISOString().slice(0, 10) + '.json';
...
function importLists(){
  ...
  try {
    ...
    if (res){ res.style.display = 'block'; res.textContent = 'Imported ' + count + ' list(s) successfully for ' + currentTenant.name + '.'; }
