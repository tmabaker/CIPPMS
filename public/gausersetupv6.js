// GA User Setup v2 - fixed response handler
// =============================================
// HARDCODED TENANT
// =============================================
...
function submitToCIPP(){
  ...
  cippFetch('/api/AddUser', 'POST', payload)
    ...
    .then(function(res){
      ...
      var msg = '';
      if (res.body && res.body.Results !== undefined)     msg = Array.isArray(res.body.Results) ? res.body.Results.join(', ') : String(res.body.Results);
      else if (res.body && res.body.result !== undefined) msg = Array.isArray(res.body.result)  ? res.body.result.join(', ')  : String(res.body.result);
      else if (res.body !== undefined)                    msg = typeof res.body === 'string'     ? res.body                   : JSON.stringify(res.body);
      var msgLower = msg.toLowerCase();
      var ok = res.ok
            || msgLower.indexOf('success') !== -1
            || msgLower.indexOf('created') !== -1
            || msgLower.indexOf('invited') !== -1;
      if (ok){
        showResult('success', 'User Created Successfully',
          'UPN: ' + upn + '\n' +
